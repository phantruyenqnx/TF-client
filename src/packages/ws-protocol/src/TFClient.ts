...
export default class TFClient {
  static SUPPORTED_SUBPROTOCOL = "foxglove.sdk.v1";
  ...
  #reconnect() {
    ...
    this.#ws.onopen = (_event) => {
      if (this.#ws.protocol !== TFClient.SUPPORTED_SUBPROTOCOL) {
        throw new Error(
          `Expected subprotocol ${TFClient.SUPPORTED_SUBPROTOCOL}, got '${
            this.#ws.protocol
          }'`,
        );
      }
  ...
  #send(message: ClientMessage) {
    this.#ws.send(JSON.stringify(message) as string);
  }
}
