...
import { ros1 } from "@tf/rosmsg-msgs-common";
import { foxgloveMessageSchemas } from "@tf/schemas/internal";
import { diffLabels, DiffObject } from "@tf/studio-base/panels/RawMessages/getDiff";
...
const tfDocsLinksByDatatype = new Map<string, string>();
for (const schema of Object.values(foxgloveMessageSchemas)) {
  const url = `#${_.kebabCase(
    schema.name,
  )}`;
  tfDocsLinksByDatatype.set(`foxglove_msgs/${schema.name}`, url);
  tfDocsLinksByDatatype.set(`foxglove_msgs/msg/${schema.name}`, url);
  tfDocsLinksByDatatype.set(`foxglove.${schema.name}`, url);
}
...
export function getMessageDocumentationLink(datatype: string): string | undefined {
  ...
  const tfDocsLink = tfDocsLinksByDatatype.get(datatype);
  if (tfDocsLink != undefined) {
    return tfDocsLink;
  }
