...
import { Time, toNanoSec } from "@tf/rostime";
import {
  Immutable,
  LayoutActions,
  MessageEvent,
...
import { BuiltinPanelExtensionContext } from "@tf/studio-base/components/PanelExtensionAdapter";
import { useAnalytics } from "@tf/studio-base/context/AnalyticsContext";
import {
  DEFAULT_SCENE_EXTENSION_CONFIG,
  SceneExtensionConfig,
} from "@tf/studio-base/panels/ThreeDeeRender/SceneExtensionConfig";
...
export function ThreeDeeRender(props: {
  ...
  useEffect(() => {
    ...
    renderer?.measurementTool.addEventListener("tf.measure-start", onStart);
    renderer?.measurementTool.addEventListener("tf.measure-end", onEnd);
    return () => {
      renderer?.measurementTool.removeEventListener("tf.measure-start", onStart);
      renderer?.measurementTool.removeEventListener("tf.measure-end", onEnd);
    };
  ...
  useEffect(() => {
    ...
    const onSubmit = (event: PublishClickEventMap["tf.publish-submit"]) => {
    ...
    renderer?.publishClickTool.addEventListener("tf.publish-start", onStart);
    renderer?.publishClickTool.addEventListener("tf.publish-submit", onSubmit);
    renderer?.publishClickTool.addEventListener("tf.publish-end", onEnd);
    return () => {
      renderer?.publishClickTool.removeEventListener("tf.publish-start", onStart);
      renderer?.publishClickTool.removeEventListener("tf.publish-submit", onSubmit);
      renderer?.publishClickTool.removeEventListener("tf.publish-end", onEnd);
    };
  }, [
    context,
    latestPublishConfig,
