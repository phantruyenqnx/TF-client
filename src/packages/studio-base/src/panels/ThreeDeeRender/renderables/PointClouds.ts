...
import { NumericType, PackedElementField, PointCloud } from "@tf/schemas";
import { SettingsTreeAction, MessageEvent } from "@tf/studio";
import { DynamicBufferGeometry } from "@tf/studio-base/panels/ThreeDeeRender/DynamicBufferGeometry";
import {
  createGeometry,
  ...
  RenderObjectHistory,
  PointsRenderable,
} from "@tf/studio-base/panels/ThreeDeeRender/renderables/pointExtensionUtils";
import type { RosObject, RosValue } from "@tf/studio-base/players/types";
...
import { POINTCLOUD_DATATYPES as TF_POINTCLOUD_DATATYPES } from "../tfSchemas";
import {
  normalizeByteArray,
  normalizeHeader,
  normalizeTime,
...
export class PointCloudHistoryRenderable extends Renderable<PointCloudHistoryUserData> {
  ...
  #validatePointCloud(pointCloud: PointCloud | PointCloud2): boolean {
    ...
    return maybeRos.header
      ...
      : this.#validateTFPointCloud(pointCloud as PointCloud);
  }

  #validateTFPointCloud(pointCloud: PointCloud): boolean {
    const data = pointCloud.data;
...
export class PointClouds extends SceneExtension<PointCloudHistoryRenderable> {
  ...
  public override getSubscriptions(): readonly AnyRendererSubscription[] {
    return [
      ...
      {
        type: "schema",
        schemaNames: TF_POINTCLOUD_DATATYPES,
        subscription: {
          handler: this.#handleTFPointCloud,
          filterQueue: this.#processMessageQueue.bind(this),
        },
      },
    ];
  ...
  #handleTFPointCloud = (messageEvent: PartialMessageEvent<PointCloud>): void => {
    ...
    if (!renderable) {
      ...
      if (settings.colorField == undefined && fieldsForTopicUpdated) {
        autoSelectColorSettings(settings, fields, {
          ...
          supportsRgbaFieldsMode: TF_POINTCLOUD_DATATYPES.has(schemaName),
        });
...
function getPose(pointCloud: PointCloud | PointCloud2): Pose {
  const maybeTF = pointCloud as Partial<PointCloud>;
  return maybeTF.pose ?? makePose();
}
