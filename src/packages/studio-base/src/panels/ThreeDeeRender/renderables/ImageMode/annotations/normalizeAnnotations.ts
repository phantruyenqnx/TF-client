...
import { Time, fromNanoSec } from "@tf/rostime";
import { ImageAnnotations, PointsAnnotationType } from "@tf/schemas";
import { RosObject } from "@tf/studio-base/players/types";
import {
  ImageMarker,
  ImageMarkerArray,
  ImageMarkerType,
} from "@tf/studio-base/types/Messages";
import { mightActuallyBePartial } from "@tf/studio-base/util/mightActuallyBePartial";
...
function normalizeTFImageAnnotations(message: Partial<ImageAnnotations>): Annotation[] {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    const style = tfPointTypeToStyle(point.type);
    if (!style) {
      continue;
    }
...
function normalizeAnnotations(message: unknown, datatype: string): Annotation[] {
  ...
  switch (datatype as (typeof ANNOTATION_DATATYPES)[number]) {
    ...
    case "foxglove.ImageAnnotations": {
      return normalizeTFImageAnnotations(message as ImageAnnotations);
    }
  }
  return [];
}
