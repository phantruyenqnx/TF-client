...
import { toNanoSec } from "@tf/rostime";
import { Grid, NumericType, PackedElementField } from "@tf/schemas";
import { SettingsTreeAction } from "@tf/studio";
import { GRID_DATATYPES } from "@tf/studio-base/panels/ThreeDeeRender/tfSchemas";
import type { RosValue } from "@tf/studio-base/players/types";
...
export type LayerSettingsTFGrid = BaseSettings &
...
const INVALID_TF_GRID = "INVALID_TF_GRID";
...
const DEFAULT_SETTINGS: LayerSettingsTFGrid = {
  visible: false,
  frameLocked: false,
  colorMode: "flat",
  minValue: undefined,
...
export type TFGridUserData = BaseUserData & {
  settings: LayerSettingsTFGrid;
  topic: string;
  tfGrid: Grid;
  mesh: THREE.Mesh;
...
export class TFGridRenderable extends Renderable<TFGridUserData> {
  ...
  #getRgbaFieldReaders(out: RgbaFieldReaders, tfGrid: Grid) {
    const { cell_stride } = tfGrid;
    for (const field of tfGrid.fields) {
      const { name } = field;
      if (name === "red") {
    ...
    for (const field of tfGrid.fields) {
      ...
      if (name === settings.colorField) {
        ...
        if (!fieldReader) {
          ...
          invalidTFGridError(this.renderer, this.userData.topic, message);
          return undefined;
        }
        return fieldReader;
      }
  ...
  public updateMaterial(settings: LayerSettingsTFGrid): void {
  ...
  public updateUniforms(tfGrid: Grid, settings: LayerSettingsTFGrid): void {
    const { material } = this.userData;
    const {
      uniforms: {
        colorMode,
    ...
    minMaxColorValues(
      tempMinMaxColor,
      settings,
      tfGrid.fields.find((field) => settings.colorField === field.name)?.type ??
        NumericType.UNKNOWN,
    );
  ...
  public updateTexture(tfGrid: Grid, settings: LayerSettingsTFGrid): void {
    let texture = this.userData.texture;
    const fieldReader = this.#getColorByFieldReader(tfGrid, settings);
    if (!fieldReader) {
      return;
    }
    ...
    const view = new DataView(
      tfGrid.data.buffer,
      tfGrid.data.byteOffset,
      tfGrid.data.byteLength,
    );

    const { column_count: cols, row_stride, cell_stride } = tfGrid;
    const rows = tfGrid.data.length / row_stride;
    ...
    if (formatChanged || sizeChanged) {
      ...
      texture.dispose();
      texture = createTexture(tfGrid, settings);
    ...
    if (floatMode) {
      ...
      if (settings.colorMode === "rgba-fields") {
        this.#getRgbaFieldReaders(tempRgbaFieldReaders, tfGrid);
...
export class TFGrid extends SceneExtension<TFGridRenderable> {
  public static extensionId = "tf.Grid";
  #fieldsByTopic = new Map<string, string[]>();

  public constructor(renderer: IRenderer, name: string = TFGrid.extensionId) {
    super(name, renderer);
  }
    ...
    return [
      {
        ...
        subscription: { handler: this.#handleTFGrid, filterQueue: onlyLastByTopicMessage },
      },
    ];
  }
  ...
  public override settingsNodes(): SettingsTreeEntry[] {
    ...
    for (const topic of this.renderer.topics ?? []) {
      ...
      const config = (configTopics[topic.name] ?? {}) as Partial<LayerSettingsTFGrid>;
  ...
  public override handleSettingsAction = (action: SettingsTreeAction): void => {
    ...
    if (renderable) {
      const settings = this.renderer.config.topics[topicName] as
        | Partial<LayerSettingsTFGrid>
        | undefined;
      renderable.userData.settings = { ...DEFAULT_SETTINGS, ...settings };

      renderable.updateMaterial(renderable.userData.settings);
      renderable.updateUniforms(renderable.userData.tfGrid, renderable.userData.settings);
      if (
        ...
        renderable.updateTexture(renderable.userData.tfGrid, renderable.userData.settings);
      }
      renderable.syncPickingMaterial();
    }
  };

  #handleTFGrid = (messageEvent: PartialMessageEvent<Grid>): void => {
    const topic = messageEvent.topic;
    const tfGrid = normalizeTFGrid(messageEvent.message);
    const receiveTime = toNanoSec(messageEvent.receiveTime);

    let renderable = this.renderables.get(topic);

    if (!this.#validateTFGrid(tfGrid, messageEvent.topic)) {
      if (renderable) {
    ...
    if (!fields || fields.length !== tfGrid.fields.length) {
      fields = tfGrid.fields.map((field) => field.name);
    ...
    if (renderable) {
      ...
      // Check color
      const texture = createTexture(tfGrid, settings);
      ...
      renderable = new TFGridRenderable(topic, this.renderer, {
        receiveTime,
        messageTime: toNanoSec(tfGrid.timestamp),
        frameId: this.renderer.normalizeFrameId(tfGrid.frame_id),
        pose: tfGrid.pose,
  ...
  #validateTFGrid(tfGrid: Grid, topic: string): boolean {
    const { cell_stride, row_stride, column_count: cols } = tfGrid;
    const rows = tfGrid.data.byteLength / row_stride;

    if (tfGrid.fields.length === 0) {
      invalidTFGridError(
        this.renderer,
        topic,
    ...
    if (Math.floor(cols) !== cols || Math.floor(rows) !== rows) {
      const message = `Grid column count (${tfGrid.column_count}) or row count (${rows} = data.byteLength ${tfGrid.data.byteLength} / row_stride ${row_stride}) is not an integer.`;
      invalidTFGridError(this.renderer, topic, message);
      return false;
    }
  ...
  /** @param tfGrid must be validated already */
  #updateTFGridRenderable(
    renderable: TFGridRenderable,
    tfGrid: Grid,
    receiveTime: bigint,
    settings: LayerSettingsTFGrid,
  ): void {
    renderable.userData.tfGrid = tfGrid;
    renderable.userData.pose = tfGrid.pose;
    renderable.userData.receiveTime = receiveTime;
    renderable.userData.messageTime = toNanoSec(tfGrid.timestamp);
    renderable.userData.frameId = this.renderer.normalizeFrameId(tfGrid.frame_id);
    const { row_stride, column_count: cols } = tfGrid;
    const rows = tfGrid.data.byteLength / row_stride;

    renderable.updateMaterial(settings);
    renderable.updateUniforms(tfGrid, settings);
    renderable.updateTexture(tfGrid, settings);

    renderable.scale.set(tfGrid.cell_size.x * cols, tfGrid.cell_size.y * rows, 1);
...
function invalidTFGridError(renderer: IRenderer, topic: string, message: string): void {
  renderer.settings.errors.addToTopic(topic, INVALID_TF_GRID, message);
}

function createTexture(tfGrid: Grid, settings: GridColorModeSettings): THREE.DataTexture {
  const { column_count: cols, row_stride } = tfGrid;
  const rows = tfGrid.data.byteLength / row_stride;
...
function normalizeTFGrid(message: PartialMessage<Grid>): Grid {
  return {
