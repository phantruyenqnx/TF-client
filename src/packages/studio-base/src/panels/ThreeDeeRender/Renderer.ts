...
import { ObjectPool } from "@tf/den/collection";
import Logger from "@tf/log";
import { Time, fromNanoSec, isLessThan, toNanoSec } from "@tf/rostime";
import type { FrameTransform, FrameTransforms, SceneUpdate } from "@tf/schemas";
import {
  Immutable,
  MessageEvent,
  ParameterValue,
...
} from "@tf/studio-base/components/PanelExtensionAdapter";
import { HUDItemManager } from "@tf/studio-base/panels/ThreeDeeRender/HUDItemManager";
import { LayerErrors } from "@tf/studio-base/panels/ThreeDeeRender/LayerErrors";
import { ICameraHandler } from "@tf/studio-base/panels/ThreeDeeRender/renderables/ICameraHandler";
import IAnalytics from "@tf/studio-base/services/IAnalytics";
import { palette, fontMonospace } from "@tf/theme";
import { LabelMaterial, LabelPool } from "@tf/three-text";
...
import { FRAME_TRANSFORMS_DATATYPES, FRAME_TRANSFORM_DATATYPES } from "./tfSchemas";
...
export class Renderer extends EventEmitter<RendererEvents> implements IRenderer {
  ...
  // eslint-disable-next-line @tf/no-boolean-parameters
  ...
  #handleFrameTransform = ({ message }: MessageEvent<DeepPartial<FrameTransform>>): void => {
    // foxglove.FrameTransform schema - Ingest this single transform into our TF tree
  ...
  #handleFrameTransforms = ({ message }: MessageEvent<DeepPartial<FrameTransforms>>): void => {
    // foxglove.FrameTransforms schema - Ingest the list of transforms into our TF tree
