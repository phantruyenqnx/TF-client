...
const extractTypeFromStudioEnumAnnotationRegex = /(.*)__(tf|webviz)_enum$/;

// TF Studio enum annotations are of the form: "Foo__tf_enum" (notice double underscore)
...
export const enumValuesByDatatypeAndField = (
  ...
  for (const [datatype, value] of datatypes) {
    ...
    for (const field of value.definitions) {
      ...
      // check if current field is annotation of the form: "Foo bar__tf_enum"
