...
import { Time, toRFC3339String } from "@tf/rostime";
import {
  AppURLState,
  updateAppURLState,
  parseAppURLState,
} from "@tf/studio-base/util/appURLState";
import isDesktopApp from "@tf/studio-base/util/isDesktopApp";

jest.mock("@tf/studio-base/util/isDesktopApp", () => ({
  __esModule: true,
  default: jest.fn(),
}));
...
describe("app state url parser", () => {
  // Note that the tf:// URL here is different from actual tf URLs because Node's URL parser
  // interprets tf:// URLs differently than the browser does.
  describe.each([
    { isDesktop: true, urlBuilder: () => new URL("tf://host/open") },
    { isDesktop: false, urlBuilder: () => new URL("#") },
  ...
  it("encodes rosbag urls", () => {
    expect(
      updateAppURLState(baseURL(), {
        ...
        ds: "ros1-remote-bagfile",
        dsParams: {
          url: "http://tf-platform.dev/test.bag",
        },
      }).href,
    ).toEqual(
      "http://example.com/?ds=ros1-remote-bagfile&ds.url=http%3A%2F%2Ftf-platform.dev%2Ftest.bag",
    );
  });
  ...
  describe("url states", () => {
    ...
    it.each<AppURLState>([
      ...
      },
      {
        ...
        dsParams: { url: "ws://tf-platform.dev:9090/test.bag", eventId },
      },
