...
import { ROS2_TO_DEFINITIONS, Rosbag2, SqliteSqljs } from "@tf/rosbag2-web";
import { stringify } from "@tf/rosmsg";
import { Time, add as addTime } from "@tf/rostime";
...
import { basicDatatypes } from "@tf/studio-base/util/basicDatatypes";
...
export class RosDb3IterableSource implements IIterableSource {
  ...
  public async initialize(): Promise<Initalization> {
    const res = await fetch(
      // tf-depcheck-used: babel-plugin-transform-import-meta
      new URL("@tf/sql.js/dist/sql-wasm.wasm", import.meta.url).toString(),
    );
    ...
    for (const topicDef of topicDefs) {
      ...
      if (parsedMsgdef == undefined) {
        problems.push({
          ...
          tip: "ROS 2 .db3 files do not contain message definitions, so only well-known ROS types are supported in TF Studio. As a workaround, you can convert the db3 file to mcap using the mcap CLI. For more information, contact support.",
        });
        continue;
      }
