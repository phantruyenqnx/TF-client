...
import { debouncePromise } from "@tf/den/async";
import Log from "@tf/log";
import { parseChannel, ParsedChannel } from "@tf/mcap-support";
import { MessageDefinition, isMsgDefEqual } from "@tf/message-definition";
import CommonRosTypes from "@tf/rosmsg-msgs-common";
import { MessageWriter as Ros1MessageWriter } from "@tf/rosmsg-serialization";
import { MessageWriter as Ros2MessageWriter } from "@tf/rosmsg2-serialization";
import { fromMillis, fromNanoSec, isGreaterThan, isLessThan, Time } from "@tf/rostime";
import { ParameterValue } from "@tf/studio";
import { Asset } from "@tf/studio-base/components/PanelExtensionAdapter";
import PlayerProblemManager from "@tf/studio-base/players/PlayerProblemManager";
import { estimateObjectSize } from "@tf/studio-base/players/messageMemoryEstimation";
import {
  AdvertiseOptions,
  MessageEvent,
  Player,
  ...
  Topic,
  TopicStats,
} from "@tf/studio-base/players/types";
import rosDatatypesToMessageDefinition from "@tf/studio-base/util/rosDatatypesToMessageDefinition";
import {
  Channel,
  ChannelId,
  ClientChannel,
...
export default class TFWebSocketPlayer implements Player {
  ...
  #open = (): void => {
    ...
    if (this.#client != undefined) {
      throw new Error(`Attempted to open a second TF WebSocket connection`);
    }
    ...
    this.#client.on("status", (event) => {
      const msg = `TFWebSocket: ${event.message}`;
  ...
  public setParameter(key: string, value: ParameterValue): void {
    if (!this.#client) {
      throw new Error(`Attempted to set parameters without a valid TF WebSocket connection`);
    }

    log.debug(`TFWebSocketPlayer.setParameter(key=${key}, value=${value})`);
  ...
  public publish({ topic, msg }: PublishPayload): void {
    if (!this.#client) {
      throw new Error(`Attempted to publish without a valid TF WebSocket connection`);
    }
  ...
  public async callService(serviceName: string, request: unknown): Promise<unknown> {
    if (!this.#client) {
      throw new Error(
        `Attempted to call service ${serviceName} without a valid TF WebSocket connection.`,
      );
    }

    if (request == undefined || typeof request !== "object") {
      throw new Error("TFWebSocketPlayer#callService request must be an object.");
    }
  ...
  public async fetchAsset(uri: string): Promise<Asset> {
    if (!this.#client) {
      throw new Error(
        `Attempted to fetch assset ${uri} without a valid TF WebSocket connection.`,
      );
    } else if (!this.#serverCapabilities.includes(ServerCapability.assets)) {
      throw new Error(`Fetching assets (${uri}) is not supported for TFWebSocketPlayer`);
    }
