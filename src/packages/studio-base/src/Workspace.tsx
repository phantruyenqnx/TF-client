...
import Logger from "@tf/log";
import { AppSetting } from "@tf/studio-base/AppSetting";
import { AppBarProps, AppBar } from "@tf/studio-base/components/AppBar";
import { CustomWindowControlsProps } from "@tf/studio-base/components/AppBar/CustomWindowControls";
import { DataSourceDialog } from "@tf/studio-base/components/DataSourceDialog";
import DocumentDropListener from "@tf/studio-base/components/DocumentDropListener";
import { EventsList } from "@tf/studio-base/components/EventsList";
import KeyListener from "@tf/studio-base/components/KeyListener";
import {
  MessagePipelineContext,
  useMessagePipeline,
  useMessagePipelineGetter,
} from "@tf/studio-base/components/MessagePipeline";
import PanelLayout from "@tf/studio-base/components/PanelLayout";
import PanelSettings from "@tf/studio-base/components/PanelSettings";
import PlaybackControls from "@tf/studio-base/components/PlaybackControls";
import { ProblemsList } from "@tf/studio-base/components/ProblemsList";
import RemountOnValueChange from "@tf/studio-base/components/RemountOnValueChange";
import { Sidebars, SidebarItem } from "@tf/studio-base/components/Sidebars";
import Stack from "@tf/studio-base/components/Stack";
import { StudioLogsSettings } from "@tf/studio-base/components/StudioLogsSettings";
import { SyncAdapters } from "@tf/studio-base/components/SyncAdapters";
import { TopicList } from "@tf/studio-base/components/TopicList";
import VariablesList from "@tf/studio-base/components/VariablesList";
import { WorkspaceDialogs } from "@tf/studio-base/components/WorkspaceDialogs";
import { useAppContext } from "@tf/studio-base/context/AppContext";
import { useCurrentUser } from "@tf/studio-base/context/BaseUserContext";
import { EventsStore, useEvents } from "@tf/studio-base/context/EventsContext";
import { useExtensionCatalog } from "@tf/studio-base/context/ExtensionCatalogContext";
...
import {
  ...
  useWorkspaceStore,
} from "@tf/studio-base/context/Workspace/WorkspaceContext";
import { useAppConfigurationValue } from "@tf/studio-base/hooks";
import { useDefaultWebLaunchPreference } from "@tf/studio-base/hooks/useDefaultWebLaunchPreference";
import useElectronFilesToOpen from "@tf/studio-base/hooks/useElectronFilesToOpen";
import { PlayerPresence } from "@tf/studio-base/players/types";
import { PanelStateContextProvider } from "@tf/studio-base/providers/PanelStateContextProvider";
import WorkspaceContextProvider from "@tf/studio-base/providers/WorkspaceContextProvider";
import { parseAppURLState } from "@tf/studio-base/util/appURLState";
...
function WorkspaceContent(props: WorkspaceProps): JSX.Element {
  ...
  const openHandle = useCallback(
    async (
      handle: FileSystemFileHandle /* tf-depcheck-used: @types/wicg-file-system-access */,
    ) => {
...
export default function Workspace(props: WorkspaceProps): JSX.Element {
  ...
  return (
    ...
    >
      <AutoConnectTFBridge isPlayerPresent={isPlayerPresent} />
      <WorkspaceContent {...props} />
    </WorkspaceContextProvider>
  );
}

function AutoConnectTFBridge(props: { isPlayerPresent: boolean }): null {
  const { selectSource } = usePlayerSelection();
  const hasConnected = useRef(false);

  useEffect(() => {
    if (!props.isPlayerPresent && !hasConnected.current) {
      hasConnected.current = true;
      void selectSource("foxglove-websocket", {
        type: "connection",
        params: { url: "ws://localhost:8765" },
      });
    }
  }, [props.isPlayerPresent, selectSource]);
