...
import { createTssReactNameTransformer } from "@tf/typescript-transformers";
...
export function makeConfig(
  ...
  return {
    resolve: {
      ...
      alias: {
        "@tf/studio-base": path.resolve(__dirname, "src"),
      },
      fallback: {
    ...
    module: {
      rules: [
        ...
        },
        {
          ...
          use: [
            {
              loader: "ts-loader", // tf-depcheck-used: ts-loader
              options: {
                transpileOnly: true,
        ...
        {
          test: /\.svg$/,
          loader: "@svgr/webpack", // tf-depcheck-used: @svgr/webpack
          options: {
            svgo: {
        ...
        { test: /\.ne$/, loader: "nearley-loader" }, // tf-depcheck-used: nearley-loader
        {
          test: /\.(png|jpg|gif)$/i,
          type: "asset",
          parser: {
        ...
        {
          test: /\.css$/,
          loader: "style-loader", // tf-depcheck-used: style-loader
          sideEffects: true,
        },
        {
          test: /\.css$/,
          loader: "css-loader", // tf-depcheck-used: css-loader
          options: { sourceMap: true },
        },
        {
          test: /\.css$/,
          loader: "esbuild-loader", // tf-depcheck-used: esbuild-loader
        ...
        { test: /\.(glb|bag|ttf|bin)$/, type: "asset/resource" },
        {
          ...
          loader: "string-replace-loader", // tf-depcheck-used: string-replace-loader
          options: {
            multiple: [
              {
                search: /etwModule\s*=\s*require\(etwModulePath\);/,
                replace:
                  "throw new Error('[TF] This module is not supported in the browser.');",
              },
              {
                search: `typescript-etw";r=require(i)`,
                replace: `typescript-etw";throw new Error('[TF] This module is not supported in the browser.');`,
              },
              {
                search:
              ...
              },
              {
                ...
                replace: `throw new Error('[TF] This module is not supported in the browser.');`,
              },
              {
                search: `getModuleResolver=function(e){let t;try{t=require(e)}`,
                replace:
                  "getModuleResolver=function(e){let t;try{throw new Error('[TF] This module is not supported in the browser.')}",
              },
            ],
          },
        },
    ...
    plugins: [
      new webpack.ProvidePlugin({
        ...
        process: ["@tf/studio-base/util/process", "default"],
        setImmediate: ["@tf/studio-base/util/setImmediate", "default"],
      }),
      new webpack.DefinePlugin({
        // Should match webpack-defines.d.ts
        ReactNull: null, // eslint-disable-line no-restricted-syntax
        TF_STUDIO_VERSION: JSON.stringify(version),
      }),
